...
for f_name in ['submission_answers.csv', 'hard_hard10_answered.csv', 'easy_normal_answered.csv']:
        ...
        with open(f_name, 'r', encoding='utf-8-sig') as f:
            ...
            for row in reader:
                if len(row) >= 2 and row[0].strip():
                    ans = row[2].strip() if len(row) >= 3 else row[1].strip()
                    if ans:
                        PERFECT_ANSWERS[row[0].strip()] = ans
